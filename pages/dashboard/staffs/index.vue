    ...
    <div class="users-container">
      <div class="right">
        <div class="total-users container-fluid">
          <span>Tất cả nhân viên: {{ totalStaff }}</span>
          <div class="search-bar">
            <form class="form-inline">
              ...
              <nuxt-link
                to="/dashboard/staffs/addStaff"
                class="btn btn-add-beer btn-primary mx-2"
                >Thêm nhân viên</nuxt-link
              >
            </form>
          </div>
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              ...
              >
                ...
                <td>{{ getTimeFormat(staff.date_joined) }}</td>
                <td
                  :class="
                    staff.is_active
                ...
                <td>{{ staff.total_confirmed || 0 }}</td>
                <td class="action">
                  <button
                    v-if="staff.is_active"
                    class="btn btn-danger"
                    @click="changeStatusAccount(staff.is_active, staff.id)"
                  >
                    Vô hiệu hóa
                  </button>
                  <button
...
export default {
  ...
  created() {
    const URL = `/account/staffs/?page=1&page_size=${this.pageSize}`
    this.getData(URL)
  },
  computed: {
    sortOption() {
  ...
  methods: {
    ...
    async getData(url) {
      ...
      if (process.client) {
        try {
          ...
          this.totalStaff = response.data.count
        } catch (err) {
          alert(err)
        }
      }
    ...
    changePage(pageNumber) {
      const URL = `/account/staffs/?page=${pageNumber}&page_size=${this.pageSize}&q=${this.searchText}&sort=${this.sortOption}`
      this.getData(URL)
    },
    search() {
      const URL = `/account/staffs/?page=1&page_size=${this.pageSize}&q=${this.searchText}&sort=${this.sortOption}`
      this.getData(URL)
    },
    sort(field) {
    ...
    async changeStatusAccount(currentStatus, staffId) {
      let URL = `/account/${staffId}`
      ...
      URL = `/account/staffs/?page=1&page_size=${this.pageSize}`
      this.getData(URL)
    },
  },
}
